...
class Payment(PaymentCommon):
    ...
    def encode_data(self, data):
        return '|'.join('%s=%s' % (str(key), str(value))
    ...
    def request(self, amount, name=None, first_name=None, last_name=None,
        ...
        data['transactionReference'] = str(transaction_id)
        data['orderId'] = orderid or str(uuid.uuid4()).replace('-', '')
        if info1:
            data['statementReference'] = str(info1)
        else:
            data['statementReference'] = data['transactionReference']
        data['amount'] = str(int(Decimal(amount) * 100))
        if email:
    ...
    def decode_data(self, data):
        data = data.split('|')
        data = [list(map(str, p.split('=', 1))) for p in data]
    ...
    def get_seal_for_json_ws_data(self, data):
        ...
        for key in sorted(data.keys()):
            if key in ('keyVersion', 'sealAlgorithm', 'seal'):
                continue
            data_to_send.append(str(data[key]))
        data_to_send_str = ''.join(data_to_send).encode('utf-8')
        return hmac.new(str(self.secret_key).encode('utf-8'), data_to_send_str, hashlib.sha256).hexdigest()
    ...
    def cancel(self, amount, bank_data, **kwargs):
        data = {}
        data['operationAmount'] = str(int(Decimal(amount) * 100))
